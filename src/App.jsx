...
export default function App() {
  ...
  const renderAdminDashboard = () => {
    ...
    // NOVA FUNÇÃO DE IMPRESSÃO - USA O PRÓPRIO DOCUMENTO E CHAMA A JANELA ORIGINAL
    const printOrder = (order) => {
      const orderDate = order.date ? new Date(order.date).toLocaleString('pt-BR') : '';
      
      // Cria a divisória para o documento a ser impresso
      const printContainer = document.createElement('div');
      printContainer.id = 'print-container-app';
      
      // Estrutura HTML do recibo
      printContainer.innerHTML = `
        <div style="font-family:'Courier New',Courier,monospace; padding:20px; max-width:400px; margin:0 auto; color:#000;">
          <h1 style="font-size:1.2rem; text-align:center; border-bottom:1px dashed #000; padding-bottom:10px;">SUA LOJA - RESERVA #${order.orderNumber || order.id}</h1>
          ...
          <div style="font-weight:bold; font-size:1.2rem; text-align:right; border-top:1px dashed #000; padding-top:10px; margin-top:10px;">TOTAL: R$ ${Number(order.total || 0).toFixed(2)}</div>
          <p style="text-align:center; font-size: 0.8rem; margin-top:20px;">Pagamento a ser realizado na retirada.</p>
        </div>
      `;

      // Regras de CSS exclusivas que dizem ao navegador para imprimir APENAS o recibo
      const style = document.createElement('style');
      style.id = 'print-style-app';
      style.textContent = `
        #print-container-app { display: none; }
        @media print {
          body > :not(#print-container-app) { display: none !important; }
          #print-container-app { display: block !important; position: absolute; left: 0; top: 0; width: 100%; background: white; z-index: 99999; }
          @page { margin: 0; }
        }
      `;

      // Adiciona o elemento e o estilo escondidos à página
      document.head.appendChild(style);
      document.body.appendChild(printContainer);

      // Aguarda um instante para a página processar os elementos e chama a janela original
      setTimeout(() => {
        window.print();
        
        // Remove os elementos da página de forma a limpar após a impressão ser fechada
        setTimeout(() => {
